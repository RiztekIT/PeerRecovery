...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "snapshotToArray2", function() { return snapshotToArray2; });
...
let ChatListPage = class ChatListPage {
    constructor(nav, chatService, firebaseAuthService) {
        this.nav = nav;
    ...
    getChats() {
        ...
        this.chatService.getUserChats().orderByChild(this.firebaseAuthService.usersign.uid).equalTo('true').on('value', (resp) => {
            let chats = snapshotToArray(resp);
            this.chats = [];
            this.keysUsers = [];
            this.chatService.chatsUsers = [];
    ...
    openChat(chat) {
        ...
        if (chat.displayName != 'chatGroup') {
            this.chatService.getChat().orderByChild(chat.uid).equalTo('true').on('value', (resp) => {
                console.log(resp);
