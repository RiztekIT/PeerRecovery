...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["pages-home-home-module"], {
  ...
  function srcAppPagesHomeHomePageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var HomePage = /*#__PURE__*/function () {
      function HomePage(util, authSVC, geolocation, nativeGeocoder, platform, zone, alertController, nav, router, modalCtr, firebaseAuthService, appointmentService) {
        ...
        value: function getUserLocation() {
          ...
          this.geolocation.getCurrentPosition().then(function (resp) {
            ...
            if (_this3.platform.is('cordova')) {
              ...
              _this3.nativeGeocoder.reverseGeocode(resp.coords.latitude, resp.coords.longitude, options).then(function (result) {
                console.log(result);
                _this3.userLocation = result[0];
                console.log(_this3.userLocation);
              })["catch"](function (error) {
                return console.log(error);
              });
            } else {
              _this3.getGeoLocation(resp.coords.latitude, resp.coords.longitude);
            }
          })["catch"](function (error) {});
          var watch = this.geolocation.watchPosition();
          watch.subscribe(function (data) {
            ...
            if (_this3.platform.is('cordova')) {
              ...
              _this3.nativeGeocoder.reverseGeocode(data.coords.latitude, data.coords.longitude, _options).then(function (result) {
                console.log(result);
                _this3.userLocation = result[0];
                console.log(_this3.userLocation);
              })["catch"](function (error) {
              ...
              _this3.getGeoLocation(data.coords.latitude, data.coords.longitude);
            }
          });
        }
      }, {
                    ...
                    return geocoder.geocode(request, function (results, status) {
                      if (status == google.maps.GeocoderStatus.OK) {
                        ...
                        _this4.zone.run(function () {
                          if (result != null) {
                            _this4.userCity = result.formatted_address;

                            if (type === 'reverseGeocode') {
                              _this4.latLngResult = result.formatted_address;
                            }
                          }
                        });
                      }
        ...
        value: function reverseGeocode(lat, lng) {
          ...
          if (this.platform.is('cordova')) {
            ...
            this.nativeGeocoder.reverseGeocode(lat, lng, options).then(function (result) {
              return _this5.userLocationFromLatLng = result[0];
            })["catch"](function (error) {
              return console.log(error);
            });
          } else {
          ...
          if (this.platform.is('cordova')) {
            ...
            this.nativeGeocoder.forwardGeocode(address, options).then(function (result) {
              _this6.zone.run(function () {
                _this6.lat = result[0].latitude;
                _this6.lng = result[0].longitude;
              });
            })["catch"](function (error) {
              return console.log(error);
            });
