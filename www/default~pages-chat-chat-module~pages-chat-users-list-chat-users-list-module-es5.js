      ...
      _createClass(ChatUsersListPage, [{
        ...
        value: function openChat(item) {
          ...
          if (!this.group) {
            ...
            this.chatService.getChat().orderByChild(item.uid).equalTo('true').once('value', function (resp) {
              console.log(resp);
              ...
              if (chats.length > 0) {
                chats.forEach(function (chat) {
                  console.log(chat);
                  var users = Object.keys(chat).filter(function (user) {
                    return user != _this2.firebaseAuthService.usersign.uid && user != 'key';
                  });
                  console.log(users);
                  ...
                  if (users.length == 1) {
                    ...
                    _this2.nav.navigateForward("/chat");
                  } else {
                    localStorage.setItem('user2', JSON.stringify(item));

                    var idchat = _this2.chatService.newChat(item);
        ...
        value: function openChatGroup() {
          ...
          var idchat = this.chatService.newChatGroup(this.groupusers);
          localStorage.setItem('idchat', idchat);
    ...
    var ChatService = /*#__PURE__*/function () {
      ...
      _createClass(ChatService, [{
          ...
          firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('/members/').orderByKey().equalTo(idchat).on('value', function (resp) {
            ...
            users.forEach(function (user) {
              ...
              var keyusers = Object.keys(user).filter(function (key) {
                return key != _this3.firebaseAuthService.usersign.uid && key != 'key';
              });
        ...
        value: function readChats() {
          ...
          firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref(this.firebaseAuthService.usersign.uid + '/messages/' + chatid).once('value', function (resp) {
            ...
            messages.forEach(function (message) {
              if (message.sender != _this4.firebaseAuthService.usersign.uid) {
                firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref(_this4.firebaseAuthService.usersign.uid + '/messages/' + chatid + '/' + message.key + '/read').set('true');
              }
            });
          });
        }
        ...
        value: function newChatGroup(users) {
          ...
          users.forEach(function (user) {
            firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref(user.uid + '/members/' + key + '/' + user.uid).set('true');
            firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref(user.uid + '/members/' + key + '/' + _this5.firebaseAuthService.usersign.uid).set('true');
            firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref('/members/' + key + '/' + user.uid).set('true');
            firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref(_this5.firebaseAuthService.usersign.uid + '/members/' + key + '/' + user.uid).set('true');
            firebase__WEBPACK_IMPORTED_MODULE_4__["default"].database().ref(user.uid + '/chats/' + key).set(chat);
          });
          return key;
        }
...
//# sourceMappingURL=default~pages-chat-chat-module~pages-chat-users-list-chat-users-list-module-es5.js.map